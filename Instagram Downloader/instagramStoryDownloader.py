...
def download():
    ...
    instance.login(user="cocaxot501", passwd="")
    ...
    for file in files:
        ...
        if extension == ".jpg" or extension == ".mp4":
            ...
            if is_exists:
                os.rename(
                    os.path.abspath(os.getcwd()) + "\\temp\\" + file,
                    desktop_location + "Insta Downloads/" + file,
                )
            else:
...
try:
    ...
    if (
        str(sys.exc_info()[1])
        == 'Login error: "fail" status, message "feedback_required".'
    ):
        download()
    else:
